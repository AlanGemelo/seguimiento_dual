<nav class="sidebar sidebar-offcanvas animate__animated animate__jackInTheBox  animate__delay-2s" id="sidebar">
    <ul class="nav">
        ...
        <!-- Verifica si existe una dirección en la sesión -->
        @isset(session()->get("direccion")->id)
            <!-- Opciones visibles para ciertos roles -->
            ...
            @endif
        @endisset
        <!-- Opciones adicionales para roles específicos -->
        @if ((Auth::user()->rol_id === 1 && session("direccion")) || Auth::user()->rol_id === 4)
            ...
            <li class="nav-item {{ request()->routeIs("empresas.*") ? "active" : "", request()->routeIs("mentores.*") ? "active" : "" }}">
                ...
                <div class="collapse" id="charts">
                    <ul class="nav flex-column sub-menu">
                        <li class="nav-item {{ request()->routeIs("empresas.*") ? "active" : "" }}"><a class="nav-link" href="{{ route("empresas.index") }}">Empresa</a></li>
                        <li class="nav-item {{ request()->routeIs("mentores.*") ? "active" : "" }}"><a class="nav-link" href="{{ route("mentores.index") }}">Mentor Industrial</a></li>
                    </ul>
                </div>
            </li>
            @if (Auth::user()->rol_id === 1 || Auth::user()->rol_id === 4)
                <li class="nav-item {{ request()->routeIs("direcciones.*") ? "active" : "", request()->routeIs("mentores.*") ? "active" : "" }}">
                    <a class="nav-link" data-bs-toggle="collapse" href="#chartsD" aria-expanded="false" aria-controls="chartsD">
                    ...
                    <div class="collapse" id="chartsD">
                        <ul class="nav flex-column sub-menu">
                            @if (Auth::user()->rol_id === 1)
                                <li class="nav-item {{ request()->routeIs("direcciones.*") ? "active" : "" }}"><a class="nav-link" href="{{ route("direcciones.index") }}">Direccion Carrera</a></li>
                                <li class="nav-item {{ request()->routeIs("directores.*") ? "active" : "" }}"><a class="nav-link" href="{{ route("directores.index") }}">Director de Carrera</a></li>
                            @endif
                            <li class="nav-item {{ request()->routeIs("carreras.*") ? "active" : "" }}"><a class="nav-link" href="{{ route("carreras.index") }}">Programa Educativo</a></li>
                        </ul>
                    </div>
                </li>
            ...
            @endif
        @endif
        <!-- Divider y opciones de perfil -->
        <divider />
        <hr>
        <li class="nav-item {{ request()->routeIs("profile.*") ? "active" : "", request()->routeIs("profile.*") ? "active" : "" }}">
            <a class="nav-link" data-bs-toggle="collapse" href="#chartsProfile" aria-expanded="false" aria-controls="chartsProfile">
                <i class="menu-icon mdi mdi-account"></i>
