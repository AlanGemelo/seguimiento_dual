        ...
        <div class="card-body">
            <form action="{{ route('anexo1_1.update', $anexo1_1->id) }}" method="POST" id="anexo1_1_form">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="institucion_educativa" name="institucion_educativa" value="{{ $anexo1_1->institucion_educativa }}" required>
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-control" id="programa_educativo_id" name="programa_educativo_id" required>
                        @foreach($carreras as $carrera)
                            <option value="{{ $carrera->id }}" {{ $anexo1_1->programa_educativo_id == $carrera->id ? 'selected' : '' }}>{{ $carrera->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <table class="table table-bordered" id="competencias_table">
                        ...
                        <tbody>
                            @foreach($anexo1_1->competencias as $index => $competencia)
                                <tr>
                                    <td><input type="text" class="form-control" name="competencias[{{ $index }}][competencia]" value="{{ $competencia['competencia'] }}" required></td>
                                    <td><input type="text" class="form-control" name="competencias[{{ $index }}][actividad]" value="{{ $competencia['actividad'] }}" required></td>
                                    <td><input type="text" class="form-control" name="competencias[{{ $index }}][asignatura]" value="{{ $competencia['asignatura'] }}" required></td>
                                    <td><input type="number" class="form-control" name="competencias[{{ $index }}][cuatrimestre]" value="{{ $competencia['cuatrimestre'] }}" min="1" max="11" required></td>
...
document.addEventListener('DOMContentLoaded', function () {
    document.getElementById('add_row').addEventListener('click', function () {
        const table = document.getElementById('competencias_table').getElementsByTagName('tbody')[0];
